...
const initMap = () => {
  // 中心点
  const centerPosition = SHENZHEN.reverse()
  // 创建城市标记点
  const fuzhou = L.marker(FUZHOU.reverse()).bindPopup('这是抚州市。')
  const shenzhen = L.marker(centerPosition).bindPopup('这是深圳市。')
...
rJcFIG214AriISLbB6B5aw`;
  ...
  // 创建基础图层 - 街道地图
  const streets = L.tileLayer(mbUrl, { id: 'mapbox/streets-v11', tileSize: 512, zoomOffset: -1, attribution: ATTRIBUTIONS });
  ...
  // 梧桐山
  const wts = L.marker([22.572784, 114.200233]).bindPopup('这里是深圳市梧桐山森林公园。');
  // 洪湖
  const hh = L.marker([22.562986, 114.120796]).bindPopup('这里是深圳市洪湖公园。');
  // 创建覆盖物图层-公园
  const parks = L.layerGroup([wts, hh]);
  // 创建基础图层-卫星地图
  const satellite = L.tileLayer(mbUrl, { id: 'mapbox/satellite-v9', tileSize: 512, zoomOffset: -1, attribution: ATTRIBUTIONS });
  // 将卫星地图添加到基础图层组
